...
const KEY = (songId) => `lyr_${songId}`;           // flat key — INVARIANT 2
const HEADERS = {
...
export async function readLyrics(songId) {
  const { data } = await readDoc(KEY(songId), null);
  return data;
}

export async function saveLyrics(songId, doc) {
  await casDoc(KEY(songId), () => ({}), (d) => {
...
export async function getLyrics(song) {
  const cached = await readLyrics(song.id);
  if (cached) {
