...
export default async () => {
  ...
  const now = Date.now();
  const from = addDays(utcToDate(now), -2);
  const to = addDays(utcToDate(now), 365);
  const artists = [];
  ...
  for (let i = 0; i < entries.length; i += 12) {
    const batch = await Promise.all(entries.slice(i, i + 12).map(async ([artistId, artist]) => {
      const [profile, events] = await Promise.all([getProfile(artistId), readEvents(artistId)]);
      const gigs = occurrencesFor(events, from, to).filter((gig) => gig.endsAt > now);
