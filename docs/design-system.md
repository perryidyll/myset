...
- **0f1 — nothing that scrolls on its own is a place to start dragging from.** The handle is the grab zone, the title and the lede (`HANDLE='.grabzone,h3,.lede'`, `vote.html:1377`); a drag never starts inside `.lyr` (`SCROLLER`, `1370`) or on a control (`CONTROL`, `1360`). The grab zone is at least 56px tall (`vote.html:332`) — a real thumb target. A drag of more than 70px, or a fast flick, closes; anything less springs back (`vote.html:1387`).
- **0f2 — a sheet freezes the page behind it.** `body.sheeting` is `position:fixed` at the offset the page was at, restored on close (`vote.html:353`, `1338–1359`; `community.html:159`, `336–358`; `venue.html:200`, `713–725`). Without it iOS scrolls the page under the sheet, which is indistinguishable from having grabbed the wrong thing.
