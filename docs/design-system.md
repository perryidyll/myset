...
- **The RSVP pill** is the chip's cousin on the show rows (`index.html` `.gig .rsvp`, `artist.html` `.gigrow .rsvp`): the `.btn-grey` ground with an orange ring and orange words while you are not going, the orange fill while you are — `--accent-2` both ways, never the pink. Since the founder's ask of 2026-09-12 it sits at the **right edge** of every row in its own column (`.rsvpcol` — not `.go`, which `app.css` owns) with "<n> going" centred beneath it; the Directions pin (38px, `.dirs`) went under the time and date, and on the artist page a ticket link is a line under the day, not a fourth column. Each page restates the pill because neither waits on `app.css`.
...
- **"View on map" on the front door** is a folded-map line icon since 2026-09-12 (`index.html` `.artistsearch svg`, 17px, centred on the label with a 7px gap) — it was the ⌖ text glyph, which sits on the baseline and cannot be sized or centred like a line icon.
