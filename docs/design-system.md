...
- **The artist page's *Listen & follow* strip** (`artist.html` `LABEL`) runs Instagram, Spotify, Apple Music, YouTube Music, Bandcamp, GoFundMe, Website — a pill per link the artist filled in, in that order, each with its brand-coloured glyph (`test/darkroom.mjs` pins the order). Bandcamp and GoFundMe joined on 2026-09-13 (decision 0061).
