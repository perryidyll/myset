...
| p08 | Enforce the plan where the server refuses | conditional | Automation | MySet server R · Artist I | Netlify | Every gated thing is refused **on the server**, not merely hidden: the monthly show cap at `startShow`, the library's size (100 / 200 / 2,000 — every song in it is live to the audience since decision 0061), team seats, own prices, named setlists, merch, hiding a post (decision 0060), the business dashboard and its two caps — band members and costs logged per show, 5 on Bar Star and 10 on Rock Star, enforced against growth so a downgrade never locks a record (decision 0065, → *The business dashboard*). `test/limits.mjs` asserts that anything **not** in `NOT_BUILT` is genuinely enforced somewhere. `src: overview §2.2; _plan.mjs; decisions 0003, 0005` |
